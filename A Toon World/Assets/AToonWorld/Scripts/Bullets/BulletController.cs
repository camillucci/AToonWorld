...
public abstract class BulletController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (_bulletBehaviourType == BulletBehavour.Parabolic)
            _bulletBehaviour = new ParabolicBullet(this.transform, GetComponent<Rigidbody2D>(), _maxBulletSpeed, _gravity);
        
        if (_bulletBehaviourType == BulletBehavour.Linear)
            _bulletBehaviour = new LinearBullet(this.transform, GetComponent<Rigidbody2D>());
    }

    public Quaternion CalculateParabola(Vector2 startPosition, Vector2 targetPosition) => _bulletBehaviour.CalculateRotation(startPosition, targetPosition);

    public void Shoot()
    {
        this.PlaySound(SoundEffects.BulletSounds.RandomOrDefault()).Forget();
        _bulletBehaviour.Shoot();
    } 
